...
trash_desktops="$CLUSTER_SCRATCH/.local/share/Trash/files"
...
trash_fallback="$CLUSTER_SCRATCH/Trash/$today"
...
performdelete() {
		    ...
		    rm -rf -- "$1"
                          ...
                          path=$(readlink -f "$1")
			  #if [ -n "$verbose" ];then echo -e "Moving bluek$norm $path to $red${trash}$norm" > "${trash}/log.txt"; fi
                          if [ -n "$verbose" ];then echo -e "Moving bluek$norm $path to $red${trash}$norm" > "${trash}/log.txt"; fi
...
asknobackup() {
  unset answer
	until [ "$answer" == "y" -o "$answer" == "n" ]; do
	  echo -e "bluek$norm could not be moved to trash. Unsafe delete (y/n)?"
	  read -n 1 answer
	done
	if [ "$answer" == "y" ]
	then
